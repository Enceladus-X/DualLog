...
export default function StatsView({
  ...
  const quarterStats = (() => {
    ...
    allMatches.forEach((m) => {
      const month = new Date(m.timestamp).getMonth() // 0-11
      const qi = Math.floor(month / 3) // 0..3
      q[qi]++
    })
    return q
  })()
      ...
      <div className="p-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="detailed" className="space-y-4">
            ...
            {/* Quarterly analysis moved to bottom, year-aware, hide empty, scrollable */}
            {(() => {
                  ...
                  <CardContent className="pt-0">
                    <div className="grid grid-cols-2 gap-3 max-h-40 overflow-y-auto">
                      {entries.map(([key, count]) => {
                        ...
                        const label = settings.language === "ko" ? `${String(year).slice(-2)}년 ${q}분기` : `Q${q} ${year}`
                        return (
                          <div
                            key={key}
                            className={`p-3 rounded-lg border ${settings.theme === "dark" ? "bg-gray-700 border-gray-600" : "bg-white border-gray-200"}`}
                          >
                            <div className={`text-xs font-bold ${settings.theme === "dark" ? "text-gray-300" : "text-gray-700"}`}>{label}</div>
                            <div className={`text-lg font-bold ${settings.theme === "dark" ? "text-white" : "text-gray-900"}`}>{count} {t.games}</div>
                          </div>
                        )
                      })}
                    </div>
          ...
          <TabsContent value="matchup" className="space-y-4">
            <div className="mb-4">
              <Select value={matchupSortBy} onValueChange={setMatchupSortBy}>
                ...
                >
                  <SelectValue placeholder={t.sortCriteria} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="winRate">{t.sortByWinRate}</SelectItem>
                  <SelectItem value="yourDeck">{t.sortByDeckName}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <Card
